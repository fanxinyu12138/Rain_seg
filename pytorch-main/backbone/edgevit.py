...
class PatchEmbed(nn.Module):
    """ Image to Patch Embedding,  img_size = (H,W)
    """

    def __init__(self, img_size=(512, 512), patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
...
class EdgeVit(nn.Module):
    ...
    def __init__(self, depth=[1, 2, 5, 3], img_size=(512, 512), in_chans=3, num_classes=1000,
        ...
        # Assuming the original patch size and embed dimensions are appropriate for the new image size

        self.patch_embed1 = PatchEmbed(
            img_size=(512, 512), patch_size=4, in_chans=in_chans, embed_dim=embed_dim[0])
        self.patch_embed2 = PatchEmbed(
            img_size=(512 // 4, 512 // 4), patch_size=2, in_chans=embed_dim[0], embed_dim=embed_dim[1])
        self.patch_embed3 = PatchEmbed(
            img_size=(512 // 8, 512 // 8), patch_size=2, in_chans=embed_dim[1], embed_dim=embed_dim[2])
        self.patch_embed4 = PatchEmbed(
            img_size=(512 // 16, 512 // 16), patch_size=2, in_chans=embed_dim[2], embed_dim=embed_dim[3])
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks1:
            x3 = blk(x2)  # 下采样4倍,(48,100,300)

        x4 = self.patch_embed2(x3)  # 下采样8倍
        for blk in self.blocks2:
            x5 = blk(x4)  # 下采样8倍,(96,50,150)

        x6 = self.patch_embed3(x5)  # 下采样16倍
        for blk in self.blocks3:
            x7 = blk(x6)  # 下采样16倍

        # x8 = self.patch_embed4(x7)  # 下采样32倍
        # for blk in self.blocks4:
        #     x9 = blk(x8)  # 下采样32倍
        #
        # x10 = self.norm(x9)
        # x11 = self.pre_logits(x10)
        return x3, x5, x7

    def forward(self, x):
        x3, x5, x7 = self.forward_features(x)  # 主干输出特征维度
